...
def get_challenge():
    ...
    exec_db("INSERT INTO challenge VALUES (?,?,?,?,?)",(address,chal,ans,size,"http://"+request.remote_addr))
...
def issue():
    ...
    if(rec):
        c_obj.issueToken(provider[0], provider[3], "http://"+request.remote_addr, 0, 440000)
